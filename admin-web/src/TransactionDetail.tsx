...
export default function TransactionDetail() {
  ...
  const handleApprove = async () => {
    ...
    if (transaction.status !== "pending" && transaction.status !== "submitted") {
      alert("未処理または申請中の取引のみ承認できます");
      return;
    }

    if (!window.confirm('この取引を承認しますか？')) return;
    ...
    try {
      ...
      alert(`承認に失敗しました: ${error?.message ?? String(error)}`);
    }
  };
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      alert(`差戻しに失敗しました: ${error?.message ?? String(error)}`);
    }
  };
